...
from CPM.infer import TryOnModel
...
def virtual_makeup(request, pk):
    ...
    style_path = os.path.join(root, makeup_image)
    input_path = os.path.join(root, user_image)
    filename = f'{pk}_{request.user.id}.png'
    
    try_on_model.execute(style_path, input_path, save_path, filename)
    # os.system(cmd)
